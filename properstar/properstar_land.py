...
class ZameenScraper(scrapy.Spider):
    ...
    # base_url = 'https://www.properstar.com/turkey/buy/land-plot'
    base_url = 'https://www.properstar.com/turkey/buy/commercial'
    # base_url = 'https://www.properstar.com/turkey/buy/building'
    ...
    def start_requests(self):
        field_names = ['title', 'price', 'location', 'highlights', 'land_type', 'area','details_url', 'image_url',
                       'date']
        with open('properstar_turkey_commercial_new.csv', 'w', encoding='utf-8') as csvfile:
    ...
    def parse(self, response):
        ...
        for card in response.css('article[class="item-adaptive card-extended"]') or response.css('article[class="item-adaptive card-basic vendor-hidden"]'):
            ...
            feature = {
                ...
                'price': card.css('span.title-price span:only-child::text')
                             .get(),
                ...
                'land_type': card.css('p[class="item-highlights"]::text')
                                    .get().split(" • ")[0],
        ...
        with open('properstar_turkey_commercial_new.csv', 'a', encoding='utf-8') as csvfile:
            # reader = csv.DictReader(csvfile, fieldnames=field_names)
            writer = csv.DictWriter(csvfile, fieldnames = field_names)
            # writer.writeheader()
